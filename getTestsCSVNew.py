...
##############GENERAL DATASET
##############
columns = ["TestID","VM", "Test", "MLmodel", "Features", "WTlower", "WTupper", "AvgRunT", "AvgRunTDiv", "InterArr", "Heuristic", "KqueueSize",  "Rand", "Solved"]
...
for dir in directories:
    ...
    for test in experiments:
        ...
        df2=""
        if testName[3]=="RealServiceTime":
            df2 = pd.DataFrame([[testID, values[1], testName[0], testName[2], "Real",values[3],values[4],values[9],values[11],values[6],values[13],values[19],values[21],solvedInst]], columns=columns)
        else:
            df2 = pd.DataFrame([[testID, values[1], testName[0], testName[2], testName[4], values[3], values[4], values[9], values[11], values[6],values[13], values[19],values[21],solvedInst]], columns=columns)
...
columns = ["TestID", "VM", "Features", "WTlower", "WTupper", "AvgRunT", "AvgRunTDiv", "InterArr", "Heuristic", "KqueueSize",  "Rand", "FCFS_R", "FCFS_RC", "SJF_R", "SJF_RC", "MIP_R", "MIP_RC"]
...
for i in range(1, testID + 1):
    ...
    for index, row in dataTest.iterrows():
        ...
        if row["Features"] == "Real":
            ...
            dfReal.loc[0].KqueueSize = row["KqueueSize"]
            dfReal.loc[0].Rand = row["Rand"]
            ...
            dfCheap.loc[0].KqueueSize = row["KqueueSize"]
            dfCheap.loc[0].Rand = row["Rand"]
            TestName = row["Test"] + "_" + row["MLmodel"] #"FCFS_R", "FCFS_RC", "SJF_R", "SJF_RC", "MIP_R", "MIP_RC"
            ...
            dfTrivial.loc[0].KqueueSize = row["KqueueSize"]
            dfTrivial.loc[0].Rand = row["Rand"]
            TestName = row["Test"] + "_" + row["MLmodel"] #"FCFS_R", "FCFS_RC", "SJF_R", "SJF_RC", "MIP_R", "MIP_RC"
