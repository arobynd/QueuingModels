...
def EvaluateContinueToExecuteMIP1(queue, simData, VMs, ArrivingInstanceTime, instanceCapTime, stopWhenQueue,  outputFile, searchTime, GAPsize, model):
    ...
    if counter>= stopWhenQueue:
        ...
        #queue = updatePrioritiesMIP(queue, outputFile, searchTime, GAPsize, instanceCapTime, VMs[vmID].nextEndTime, simData,model, len(VMs))
        queue = updatePrioritiesMIP(queue, outputFile, searchTime, GAPsize, instanceCapTime, getVM_CSV(VMs),simData, model, len(VMs))
...
##############################
#
# def updatePrioritiesMIP(queue, outputFile, searchTime, GAPsize, instanceCapTime, lastEndTime, simData, model="model1", dequeueWhenNotScheduledMIP=0, machines=1):
#     auxQueue = Queue.PriorityQueue()
...
def updatePrioritiesMIP(queue, outputFile, searchTime, GAPsize, instanceCapTime, nextEndTimeCSV, simData, model="model1", dequeueWhenNotScheduledMIP=0, machines=1):
    auxQueue = Queue.PriorityQueue()
    finalQueue = Queue.PriorityQueue()
    print "Executing MIP with:"
...
def MIPsimulateInstanceArrivals_NaiveStrategy_Regression_Classification(inputData, outputFile, VMs, schedulingPolicy, instanceCapTime, groupSize=5, searchTime=120, GAPsize=0.1, model="model1"):
    ...
    for index, row in simData.iterrows():
        ...
        if (index % groupSize == 0 and q.qsize()>1):
            #print "index:", index
            vmID = getVMwithSmallestEndTime(VMs)
            #q = updatePrioritiesMIP(q, outputFile, searchTime, GAPsize, instanceCapTime, VMs[vmID].nextEndTime, simData, model, len(VMs))
            q = updatePrioritiesMIP(q, outputFile, searchTime, GAPsize, instanceCapTime, getVM_CSV(VMs), simData, model, len(VMs))
...
def MIPsimulateInstanceArrivals_HeuristicStrategy_Regression(inputData, outputFile, VMs, schedulingPolicy, instanceCapTime, groupSize=5, searchTime=120, GAPsize=0.1, model="model1", stopWhenQueue=2, dequeueWhenNotScheduledMIP=0):
    ...
    for index, row in simData.iterrows():
        ...
        if (index % groupSize == 0 and q.qsize()>1):
            ...
            vmID = getVMwithSmallestEndTime(VMs)
            #q = updatePrioritiesMIP(q, outputFile, searchTime, GAPsize, instanceCapTime, VMs[vmID].nextEndTime, simData, model, dequeueWhenNotScheduledMIP, len(VMs))
            q = updatePrioritiesMIP(q, outputFile, searchTime, GAPsize, instanceCapTime, getVM_CSV(VMs) , simData, model, dequeueWhenNotScheduledMIP, len(VMs))
