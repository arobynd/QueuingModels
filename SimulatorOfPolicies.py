...
def HeuristicEvaluateContinueToExecute1Q(queue, simData, VMs, ArrivingInstanceTime, instanceCapTime, heuristicH, useClassification = False):
...
def HeuristicEvaluateContinueToExecute2Q(queue, stoppedQueue, schedulingPolicy, simData, VMs, ArrivingInstanceTime, instanceCapTime, heuristicH, useClassification = False):
    vmID = getVMwithSmallestEndTime(VMs)
    auxQueue = Queue.PriorityQueue()
    ...
    if counter>= heuristicH:
        ...
        if (simData.loc[OldInstanceID, "TimeServiceEnds"] != simData.loc[OldInstanceID, "TimeServiceBegins"]) and (ArrivingInstanceTime >= simData.loc[OldInstanceID, "TimeServiceBegins"]):
            ...
            simData.loc[OldInstanceID, "TimeInstanceInSystem"] = ArrivingInstanceTime - simData.loc[OldInstanceID, "ArrivalTime"]
            VMs[vmID].nextEndTime = ArrivingInstanceTime
            auxOldInstance = assignPriorityForScheduling(OldInstanceID, simData.loc[OldInstanceID], schedulingPolicy , VMs[vmID].nextEndTime + 1)
...
def simulateInstanceArrivals_HeuristicStrategy_Regression1Q(inputData, outputFile, VMs, schedulingPolicy, instanceCapTime, heuristicH=2):
    ...
    for index, row in simData.iterrows():
        ...
        if VMs[vmID].nextEndTime >= ArrivingInstance.ArrivalTime:
            ...
            if heuristicH > 0:
                q = HeuristicEvaluateContinueToExecute1Q(q, simData, VMs, ArrivingInstance.ArrivalTime, instanceCapTime, heuristicH, useClassification=False)
        else:
...
##############################
##############################
def simulateInstanceArrivals_HeuristicStrategy_Regression_Classification1Q(inputData, outputFile, VMs, schedulingPolicy, instanceCapTime, heuristicH=2):
    simData = pd.read_csv(inputData, index_col=0)
    simData["Stopped"] = 0
    simData["QueuedInstances"] = 0
    q = Queue.PriorityQueue()
    for index, row in simData.iterrows():
        ...
        if VMs[vmID].nextEndTime >= ArrivingInstance.ArrivalTime:
            q.put(ArrivingInstance)
            if heuristicH > 0:
                q = HeuristicEvaluateContinueToExecute1Q(q, simData, VMs, ArrivingInstance.ArrivalTime, instanceCapTime, heuristicH, useClassification=True)
...
def simulateInstanceArrivals_HeuristicStrategy_Regression2Q(inputData, outputFile, VMs, schedulingPolicy, instanceCapTime, heuristicH=2):
    simData = pd.read_csv(inputData, index_col=0)
    simData["Stopped"] = 0
    simData["QueuedInstances"] = 0
    q = Queue.PriorityQueue()
    ...
    for index, row in simData.iterrows():
        ...
        while not q.empty() and VMs[vmID].nextEndTime < ArrivingInstance.ArrivalTime:
            ...
            if not q.empty():
                ...
                VMs=beginToProcessInstance_R(q, QueuedInstance, simData, VMs, vmID, instanceCapTime)
                vmID = getVMwithSmallestEndTime(VMs)
            if (q.empty()) and (not stoppedQueue.empty()): ######Si q está vacia y VMs[vmID].nextEndTime < ArrivingInstance.ArrivalTime...el sistema estará idle y stopped no está vacío
                if (schedulingPolicy =="SJF"):
                    stoppedQueue = sortBySJF(stoppedQueue)
                stoppedInstance = stoppedQueue.get()
                q.put(stoppedInstance)
        ...
        if VMs[vmID].nextEndTime >= ArrivingInstance.ArrivalTime:
            q.put(ArrivingInstance)
            if heuristicH > 0:
                q = HeuristicEvaluateContinueToExecute2Q(q, stoppedQueue, schedulingPolicy, simData, VMs, ArrivingInstance.ArrivalTime, instanceCapTime, heuristicH, useClassification=False)
        else:
            VMs = beginToProcessInstanceSystemIsIDLE(VMs, vmID, ArrivingInstance, simData, instanceCapTime)
...
def simulateInstanceArrivals_HeuristicStrategy_Regression_Classification2Q(inputData, outputFile, VMs, schedulingPolicy, instanceCapTime, heuristicH=2):
    simData = pd.read_csv(inputData, index_col=0)
    simData["Stopped"] = 0
    simData["QueuedInstances"] = 0
    q = Queue.PriorityQueue()
    ...
    for index, row in simData.iterrows():
        ...
        while not q.empty() and VMs[vmID].nextEndTime < ArrivingInstance.ArrivalTime:
            ...
            if not q.empty():
                ...
                VMs=beginToProcessInstance_R_C(q, QueuedInstance, simData, VMs, vmID, instanceCapTime)
                vmID = getVMwithSmallestEndTime(VMs)
            elif (q.empty()) and (not stoppedQueue.empty()):
                if (schedulingPolicy =="SJF"):
                    stoppedQueue = sortBySJF(stoppedQueue)
                stoppedInstance = stoppedQueue.get()
                q.put(stoppedInstance)
        ...
        if VMs[vmID].nextEndTime >= ArrivingInstance.ArrivalTime:
            q.put(ArrivingInstance)
            if heuristicH > 0:
                q = HeuristicEvaluateContinueToExecute2Q(q,  stoppedQueue, schedulingPolicy, simData, VMs, ArrivingInstance.ArrivalTime, instanceCapTime, heuristicH, useClassification=True)
        elif (ArrivingInstance.PredictedSolvable != 0):
            VMs = beginToProcessInstanceSystemIsIDLE(VMs, vmID, ArrivingInstance, simData, instanceCapTime)
        else:
            VMs = doNotProcessInstance(VMs, vmID, ArrivingInstance, simData)
